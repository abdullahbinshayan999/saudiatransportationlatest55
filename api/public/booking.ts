...
export default async function (request: Request) {
  ...
  try {
    ...
    if (!parsed.success) {
      return new Response(JSON.stringify({ ok: false, error: "Invalid input", issues: parsed.error.issues }), { status: 400, headers: { "Content-Type": "application/json" } });
    }
    const d = parsed.data;

    const RESEND_API_KEY = process.env.RESEND_API_KEY;
    if (!RESEND_API_KEY) {
      return new Response(JSON.stringify({ ok: false, error: "Email service not configured" }), { status: 500, headers: { "Content-Type": "application/json" } });
    }
    ...
    if (!res.ok) {
      ...
      return new Response(JSON.stringify({ ok: false, error: "Failed to send email" }), { status: 502, headers: { "Content-Type": "application/json" } });
    }
